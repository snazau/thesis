...
def get_best_threshold(experiment_dir, subject_keys, filter_method='median', k_size=7, sfreq=128, verbose=0):
    ...
    for threshold_idx, threshold in enumerate(threshold_range):
        ...
        if verbose:
            print(f'threshold = {threshold:3.2f} f1_score = {metric_meter.meters["f1_score"].avg:.4f} best_threshold = {best_threshold:3.2f} best_f1_score = {best_metric_meter.meters["f1_score"].avg:.4f}')
